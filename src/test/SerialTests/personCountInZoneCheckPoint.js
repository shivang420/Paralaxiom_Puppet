...
beforeAll(async () => {
    ...
    await page.type('.card-body #inputAddress', PersonCountInZoneCheckPointName);

    await page.select('.card-body #inputState', "Person_Count_in_Zone")
    ...
    //setting time
    await page.click('#root > div > div > div:nth-child(3) > div > div > div > div.row.mt-2 > div.col-lg-4 > div > div > div.card-body.p-1 > div > div > div:nth-child(3) > div.ml-2.mr-2.pl-2.pr-2.mt-2.md-2 > div > input', clickCount = 3)
    await page.keyboard.press('Backspace');
    await page.type('#root > div > div > div:nth-child(3) > div > div > div > div.row.mt-2 > div.col-lg-4 > div > div > div.card-body.p-1 > div > div > div:nth-child(3) > div.ml-2.mr-2.pl-2.pr-2.mt-2.md-2 > div > input', '10');
    ...
    await page.waitFor(2000);
    await page.screenshot({path: 'addingPersonCountInZoneCheckPoint.png'});
...
describe("Adding Person Count In Zone CheckPoint", () => {
    test("Login", async () => {
        expect(this.loginCheckUrlCa).toBe("http://20.188.101.204/");
    }, timeout);

    test("Person Count In Zone CheckPoint Added", async () => {
        expect(this.checkpointAdded).toBe(true);
    }, timeout);

    test("Person Count In Zone CheckPoint Edited", async () => {
        expect(this.checkpointEdited).toBe(true);
    }, timeout);

    test("Person Count In Zone CheckPoint Deleted", async () => {
