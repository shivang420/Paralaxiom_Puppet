...
beforeAll(async () => {
    ...
    await page.type('.card-body #inputAddress', LineCheckPointName);

    await page.select('.card-body #inputState', "Line")
    await page.select('div .mb-1 > #inputState', "line")
    await page.waitFor(2000);
    await page.select('#inputStatePuppet', "Medium");
    
    await page.waitFor(200);
    await page.mouse.click(433, 220);
    await page.waitFor(200);
    await page.mouse.click(1000, 220);
    ...
    await page.waitFor(2000);
    await page.screenshot({path: 'addingLineCheckPoint.png'});
...
describe("Adding Line CheckPoint", () => {
    test("Login", async () => {
        expect(this.loginCheckUrlCa).toBe("http://20.188.101.204/");
    }, timeout);

    test("Line CheckPoint Added", async () => {
        expect(this.checkpointAdded).toBe(true);
    }, timeout);

    test("Line CheckPoint Edited", async () => {
        expect(this.checkpointEdited).toBe(true);
    }, timeout);

    test("Line CheckPoint Deleted", async () => {
