...
beforeAll(async () => {
    ...
    await page.type('.card-body #inputAddress', AnprZoneCheckPointName);

    await page.select('.card-body #inputState', "Anpr_Zone")
    await page.select('div .mb-1 > #inputState', "poly")
    await page.waitFor(2000);
    await page.select('#inputStatePuppet', "Medium");
    
    await page.waitFor(200);
    await page.mouse.click(390, 290);
    await page.waitFor(200);
    await page.mouse.click(940, 290);
    await page.waitFor(200);
    await page.mouse.click(940, 640);
    await page.waitFor(200);
    await page.mouse.click(390, 640);
    await page.waitFor(200);
    await page.mouse.click(390, 290);
    await page.mouse.click(390, 290);
    ...
    await page.waitFor(2000);
    await page.screenshot({path: 'addingAnprZoneCheckPoint.png'});
...
describe("Adding ANPR Zone CheckPoint", () => {
    test("Login", async () => {
        expect(this.loginCheckUrlCa).toBe("http://20.188.101.204/");
    }, timeout);

    test("ANPR Zone CheckPoint Added", async () => {
        expect(this.checkpointAdded).toBe(true);
    }, timeout);

    test("ANPR Zone CheckPoint Edited", async () => {
        expect(this.checkpointEdited).toBe(true);
    }, timeout);

    test("ANPR Zone CheckPoint Deleted", async () => {
