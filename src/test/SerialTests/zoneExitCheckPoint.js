...
beforeAll(async () => {
    ...
    await page.type('.card-body #inputAddress', ZoneExitCheckPointName);
    ...
    await page.waitFor(2000);
    await page.screenshot({path: 'addingZoneExitCheckPoint.png'});
...
describe("Adding Zone Exit CheckPoint", () => {
    test("Login", async () => {
        expect(this.loginCheckUrlCa).toBe("http://20.188.101.204/");
    }, timeout);

    test("Zone Exit CheckPoint Added", async () => {
        expect(this.checkpointAdded).toBe(true);
    }, timeout);

    test("Zone Exit CheckPoint Edited", async () => {
        expect(this.checkpointEdited).toBe(true);
    }, timeout);

    test("Zone Exit CheckPoint Deleted", async () => {
