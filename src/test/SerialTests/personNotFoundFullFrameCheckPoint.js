...
beforeAll(async () => {
    ...
    await page.type('.card-body #inputAddress', PersonNotFoundFullFrameCheckPointName);

    await page.select('.card-body #inputState', "Person Not Found in Frame")
    ...
    await page.waitFor(2000);
    await page.screenshot({path: 'addingPersonNotFoundFullFrameCheckPoint.png'});
...
describe("Adding Person Not Found Full Frame CheckPoint", () => {
    test("Login", async () => {
        expect(this.loginCheckUrlCa).toBe("http://20.188.101.204/");
    }, timeout);

    test("Person Not Found Full Frame CheckPoint Added", async () => {
        expect(this.checkpointAdded).toBe(true);
    }, timeout);

    test("Person Not Found Full Frame CheckPoint Deleted", async () => {
