...
beforeAll(async () => {
    ...
    await page.type('.card-body #inputAddress', TrackObjectCheckPointName);

    await page.select('.card-body #inputState', "Track Object")
    await page.select('div .mb-1 > #inputState', "rectangle")
    ...
    await page.waitFor(2000);
    await page.screenshot({path: 'addingTrackObjectCheckPoint.png'});
...
describe("Adding Track Object CheckPoint", () => {
    test("Login", async () => {
        expect(this.loginCheckUrlCa).toBe("http://20.188.101.204/");
    }, timeout);

    test("Track Object CheckPoint Added", async () => {
